	...
	while len(list_of_walls) > 0:
		...
		if (y % 2) == 0:
			...
			if maze_map[x - 1, y] == UNCONNECTED:
				...
				pm("unconnected space to the left")
				...
				list_of_walls.append((x + 1, y + 1))
				list_of_walls.append((x + 2, y))
				pm("unconnected space to the right")
			else:
			...
			if maze_map[x, y - 1] == UNCONNECTED:
				...
				pm("unconnected space to the above")
				...
				list_of_walls.append((x + 1, y + 1))
				list_of_walls.append((x, y + 2))
				pm("unconnected space to the below")
			else:
				# both spaces already connected, do nothing
				pass
...
if __name__ == "__main__":
	make_maze(11, 11, 1)
	img = Image.open("images/frozen.jpg")
	(_, _, width, height) = img.getbbox()
	size = min(width, height)
	margin = (width - size) / 2
	img = img.crop((margin, 0, size + margin, size))
	img = img.resize((500, 500))
	m = Maze(11, 11, 1)
	m.overlay(img, (0, 0, 1.0, 1.0), 1)
	img.save("sample.png")
